    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        ...
        try {
            ...
            for (MethodNode method : classNode.methods) {
                if (SHUTDOWN_METHOD.containsKey(method.name) &&
                    method.desc.equals(SHUTDOWN_METHOD.get(method.name))) {

                    InsnList call = new InsnList();
                    call.add(new MethodInsnNode(INVOKESTATIC,
                        "dev/kostromdan/mods/crash_assistant/forge_coremod/CrashAssistantHooks",
                        "onMinecraftShutdown",
                        "()V",
                        false));

                    for (AbstractInsnNode insn = method.instructions.getLast();
                         insn != null;
